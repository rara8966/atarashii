...
public class DocumentAnalysisService {
    ...
    public DocumentAnalysisResponse analyze(MultipartFile file, String targetStep, String fallbackStep, AiConfig aiConfig) {
        ...
        String text = normalize(parsed.text());
        ...
        List<ExtractedField> fields = attachSourceFile(extractFields(text), fileId);
        ...
        DocumentAnalysisResponse response = new DocumentAnalysisResponse(
            ...
            workspaceStateService.mergeAnalysis(response);
            return response;
    }
    ...
    private AiAdvice buildAiAdvice(String text, String targetStep, String documentType, List<PolicyCheck> checks, List<ExtractedField> fields, AiConfig aiConfig) {
        ...
        String prompt = buildPrompt(text, targetStep, documentType, checks, fields);
        ...
        if (aiConfig != null && "deepseek".equalsIgnoreCase(aiConfig.provider()) && aiConfig.deepseekApiKey() != null && !aiConfig.deepseekApiKey().isBlank()) {
            DeepSeekClient.GenerationResult deepSeek = deepSeekClient.generate(prompt, aiConfig.deepseekApiKey(), aiConfig.deepseekModel());
        ...
        OllamaClient.GenerationResult generation = ollamaClient.generate(prompt);
    ...
    private String buildPrompt(String text, String targetStep, String documentType, List<PolicyCheck> checks, List<ExtractedField> fields) {
        ...
        String compactText = text.length() > 2_400 ? text.substring(0, 2_400) : text;
        return "你是建设用地报批审查报告助手。请基于政策明白卡、1009号模板和上传材料，找出本步骤重点修改点。\n"
                + "要求: 用中文输出，面向项目经理和客户，别太术语化。分成四段: 1材料摘要 2需要改 3可补充 4风险提醒。\n"
                + "目标步骤: " + targetStep + "\n"
                + "文档类型: " + documentType + "\n"
                + "政策依据摘录:\n" + knowledge + "\n"
                + "已抽字段: " + fields + "\n"
